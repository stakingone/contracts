...
contract TOMBFTMSpookyAutoCompounder is TShareRewardPool {
    ...
    function set_addresses(address _tomb, address _wftm, address _tshare_address, address _tomb_wftm, address _tshare_pool, address _fee_wallet, address[] memory _path1, address[] memory _path2, address payable _spooky) public{
        //must write so it is set up just once
        //must add unstaking fee to stake1 holders
	    tomb = _tomb;
	    wftm = _wftm;
    ...
    uint256 public shares_circ = 0;

    function stake(uint256 amount) public{
        uint256 amount_to_mint;
        uint256 pool_balance;

        pool_balance = IERC20(tomb_wftm).balanceOf(address(this));

	    if(shares_circ == 0){
		    amount_to_mint = amount;
	    }else{
            amount_to_mint = shares_circ*amount.div(pool_balance);
        }
    ...
    function unstake(uint256 amount) public{
        ...
        pool_balance = IERC20(tomb_wftm).balanceOf(address(this));
	    amount_to_give = pool_balance*amount.div(shares_circ);
        ...
        if(ftmtomb_staker[msg.sender].shares>=amount){
            ...
            shares_circ -= amount_to_give;
	        IERC20(tomb_wftm).transfer(msg.sender, amount_to_give);
        }
    }

    function harvest_and_compound() public{
        uint256 harvested;
        uint256 incentive;
        uint256 half_tshares;
        uint256 deadline;
        uint256 tomb_balance;
        uint256 lp_balance;
        ...
        half_tshares = (harvested-incentive).div(2);
        UniswapV2Router02(spooky).swapExactTokensForETH(half_tshares, 0, path1, msg.sender, deadline);
        UniswapV2Router02(spooky).swapExactTokensForETH(half_tshares, 0, path2, msg.sender, deadline);
        tomb_balance = IERC20(tomb).balanceOf(address(this));
        UniswapV2Router02(spooky).addLiquidityETH(tomb, tomb_balance, 0, 0, msg.sender, deadline);
